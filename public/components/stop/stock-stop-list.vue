                ...
                <div class="weui-tab__panel">
                    <div class="weui-cells">
                        <a class="weui-cell weui-cell_access" href="javascript:;" v-for="item in list" @tap="toDetail(item)">
                            ...
                            <div class="weui-cell__ft">{{item["证券简称"]}}</div>
                        </a>
                    </div>
                </div>
            </div>
    ...
    export default {
        ...
        methods: {
            ...
            getList(tabIndex, pageIndex) {
                ...
                services.stopService.getList({tab, page})
                    .then(res => {
                        if(res.code === 2000) {
            ...
            toDetail(item) {
                const query = {
                    reason: item["停牌原因"],
                    start: encodeURIComponent(item["停牌开始时间"]),
                    end: encodeURIComponent(item["停牌截止时间"]),
                    expire: item["停牌期限"],
                    code: item["证券代码"],
                    name: item["证券简称"],
                    market: item["市场"],
